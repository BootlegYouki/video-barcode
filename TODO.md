...
- [ ] **Bake Timestamp into Recorded Video (FFmpeg)**: Install `ffmpeg-kit-react-native` and post-process the recorded `.mp4` video after recording stops. Use FFmpeg's `drawtext` video filter to overlay the session date and time directly onto the video frames (pixels) so the timestamp is permanently burned into the exported file.
